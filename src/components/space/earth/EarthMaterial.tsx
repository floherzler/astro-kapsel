...
const DEFAULT_SUN_DIRECTION = new THREE.Vector3(-2, 0.5, 1.5).normalize();
...
export function EarthMaterial({ sunDirection = DEFAULT_SUN_DIRECTION }: { sunDirection?: THREE.Vector3 }) {
  const normalizedSun = useMemo(() => sunDirection.clone().normalize(), [sunDirection]);
  const [dayMap, nightMap, cloudsMap] = useLoader(THREE.TextureLoader, [
    "/earth/textures/earth-daymap-4k.jpg",
    "/earth/textures/earth-nightmap-4k.jpg",
    "/earth/textures/earth-clouds-4k.jpg",
  ]);
  ...
  const material = useMemo(
    ...
    [dayMap, nightMap, cloudsMap, normalizedSun]
  );
